...
export function autoRuntimeModeSelectionIssue(input: {
  ...
  const provider =
    ...
    inferLegacyProviderKindFromInstanceId(input.modelSelection.instanceId);
  if (!provider || !providerSupportsAutoRuntimeMode(provider)) {
    return provider
      ? unsupportedAutoRuntimeModeMessage(provider)
      : "The selected provider account does not support Auto runtime mode.";
  }
  if (
