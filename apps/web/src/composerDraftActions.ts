  ...
  (set, get) => ({
    ...
    setStickyModelSelection: (modelSelection) => {
      ...
      set((state) => {
        ...
        if (Equal.equals(state.stickyModelSelectionByProvider, nextMap)) {
          return state.stickyActiveProvider === selectionKey
            ? state
            : { stickyActiveProvider: selectionKey };
        }
        return {
    ...
    applyStickyState: (threadId) => {
      ...
      set((state) => {
        ...
        for (const [instanceId, selection] of Object.entries(stickyMap)) {
          if (selection) {
            const key = instanceId as ProviderInstanceId;
