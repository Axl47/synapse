...
export default function Sidebar() {
  ...
  const prefetchModelsForProjectNewThread = useCallback(
    (projectId: ProjectId, options?: { includeDroid?: boolean }) => {
      ...
      const provider = resolveNewThreadModelPrefetchProvider({
        ...
        projectDefaultProvider: project.defaultModelSelection
          ? inferLegacyProviderKindFromModelSelection(project.defaultModelSelection)
          : null,
  ...
  const handleAdoptExternalThread = useCallback(
    async (candidate: ExternalThreadCandidate, chosenProjectId?: ProjectId) => {
      ...
      try {
        const api = ensureNativeApi();
        const result = await api.orchestration.adoptExternalThread(adoptionInput);
  ...
  const handleRelocateProjectFolder = useCallback(
    async (projectId: ProjectId) => {
      ...
      try {
        await api.orchestration.dispatchCommand(relocateCommand);
        toastManager.add({
          type: "success",
