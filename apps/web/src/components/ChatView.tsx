...
export default function ChatView({
  ...
  const storedPrompt = composerDraft.prompt;
  const promptRef = useRef(storedPrompt);
  const prompt = storedPrompt;
  useEffect(() => {
    promptRef.current = storedPrompt;
  }, [storedPrompt, threadId]);
    ...
    () =>
      crossTaskSourceThreadId
        ? {
            ...
            sourceProvider: crossTaskSourceThread
              ? (crossTaskSourceThread.session?.provider ??
                inferLegacyProviderKindFromModelSelection(crossTaskSourceThread.modelSelection))
              : null,
          }
        : null,
    [crossTaskSourceThread, crossTaskSourceThreadId],
  );
  ...
  const persistedClaudeSupportsAutoMode =
    selectedProvider === "claudeAgent"
      ? draftModelSelectionForSelectedProvider !== null &&
          inferLegacyProviderKindFromModelSelection(draftModelSelectionForSelectedProvider) ===
            "claudeAgent" &&
  ...
  useEffect(() => {
    ...
    if (
      promptHistoryNavigationRef.current !== null &&
      nextPrompt !== promptHistoryAppliedPromptRef.current
    ) {
  ...
  const onProviderModelSelect = useCallback(
    async (provider: ProviderKind, model: ModelSlug, instanceId?: ProviderInstanceId) => {
      ...
      const nextModelSelection = buildModelSelection(provider, resolvedModel, undefined, {
        ...
        supportsAutoMode: provider === "claudeAgent" ? runtimeModel?.supportsAutoMode : undefined,
      });
  ...
  const onSend = async (
    ...
    await (async () => {
      ...
      const liveProviderForSteerGate =
        activeThread?.session?.provider ?? selectedModelSelectionProviderForSend;
  ...
  const onToggleActivePendingUserInputOption = useCallback(
    ...
    [activePendingUserInput, activePendingUserInputKey, syncComposerPromptHasSendableText],
  );
  ...
  async function onSubmitPlanFollowUp({
    ...
    const dispatchPlanFollowUpTurn = async () => {
      ...
      const livePlanProviderForSteerGate =
        activeThread?.session?.provider ?? providerForPlanDispatch;
    ...
    >
      {showContainerChatWorkspacePicker ? (
        ...
        <ProjectPicker
          align="start"
          side="top"
          triggerClassName="h-auto min-h-7 py-1 sm:h-auto"
