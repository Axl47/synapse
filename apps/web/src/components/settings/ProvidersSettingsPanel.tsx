...
function ProviderProfilesEditor(props: AppSettingsBinding) {
  const addInstance = (provider: ProviderKind): void => {
    ...
    let index = 2;
    let rawId = `${prefix}_${index}`;
    while (existingIds.has(rawId)) {
      index += 1;
      rawId = `${prefix}_${index}`;
    }
  ...
  const updateInstance = (
    ...
    fallback: ProviderInstanceConfig,
  ...
  return (
    <div className="mt-4 space-y-3">
      {PROVIDER_DESCRIPTORS.map(({ kind: provider, displayName }) => {
        const instances = getManageableProviderInstances(props.settings, provider);
        return (
          <div key={provider} className={cn(SETTINGS_INSET_LIST_CLASS_NAME, "p-3")}>
            <div className="flex items-center justify-between gap-3">
              <div>
                <div className="text-xs font-medium text-foreground">{displayName} profiles</div>
                <div className="text-xs text-muted-foreground">
                  Separate launch identity, model favorites, and environment.
                </div>
              </div>
              <Button type="button" size="xs" variant="outline" onClick={() => addInstance(provider)}>
                Add profile
              </Button>
            </div>
            {instances.length > 0 ? (
              <div className="mt-3 space-y-3">
                {instances.map(({ instanceId, instance }) => (
                  <div key={instanceId} className="space-y-3 border-t border-border/70 pt-3">
                    <div className="flex items-center gap-2">
                      <DebouncedSettingTextInput
                        id={`provider-profile-${instanceId}-name`}
                        size="sm"
                        variant="soft"
                        className="min-w-0 flex-1"
                        value={instance.displayName ?? ""}
                        onCommit={(displayName) =>
                          updateInstance(instanceId, instance, { displayName })
                        }
                      ...
                      <Switch
                        ...
                        onCheckedChange={(enabled) =>
                          updateInstance(instanceId, instance, { enabled })
                        }
                    ...
                    <ProviderInstanceEnvironmentEditor
                      ...
                      onChange={(environment: ProviderInstanceEnvironment) =>
                        updateInstance(instanceId, instance, { environment })
                      }
                    />
                  </div>
                ))}
...
export function ProvidersSettingsPanel({
  ...
  return (
    <div className="space-y-6">
      ...
      <SettingsSection title="Provider profiles">
        <SettingsRow
          title="Accounts and launch profiles"
          description="Run multiple isolated accounts for the same provider without sharing model choices or credentials."
        >
