...
export function modelSelectionsEqual(left: ModelSelection, right: ModelSelection): boolean {
  const leftProvider = inferLegacyProviderKindFromModelSelection(left);
  const rightProvider = inferLegacyProviderKindFromModelSelection(right);
  return (
    leftProvider === rightProvider &&
    left.instanceId === right.instanceId &&
    left.model === right.model &&
    JSON.stringify(left.options ?? null) === JSON.stringify(right.options ?? null) &&
    (leftProvider !== "claudeAgent" ||
      rightProvider !== "claudeAgent" ||
