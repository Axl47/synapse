...
describe("ChatView timeline estimator parity (full app)", () => {
    ...
    try {
      ...
      await vi.waitFor(() => {
        expect(
          useComposerDraftStore.getState().draftsByThreadId[THREAD_ID]?.modelSelectionByProvider
            .codex,
        ).toMatchObject({ instanceId: "codex", model: "gpt-5.5" });
      });
      ...
      await vi.waitFor(() => {
        expect(
          useComposerDraftStore.getState().draftsByThreadId[THREAD_ID]?.modelSelectionByProvider
            .codex,
        ).toMatchObject({ instanceId: "codex", model: "gpt-5.2" });
      });
    } finally {
      await mounted.cleanup();
    }
  ...
  it("shows the skinny inline plan card for active turn plans", async () => {
    ...
    try {
      ...
      // Active plan activity shares the centered queued-follow-up rail and spans the composer
      // width while the input keeps its rounded top corners.
