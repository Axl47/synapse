...
let instanceQueryCacheKey = "";
let instanceQueryCacheResult: ReadonlyArray<QueryResultLike> = [];
...
beforeEach(() => {
  modelQueries.clear();
  agentQueries.clear();
  instanceQueryCacheKey = "";
  instanceQueryCacheResult = [];
  ...
  mocks.useQueries.mockReset().mockImplementation(
    (value: { readonly queries: ReadonlyArray<QueryOptionsLike> }) => {
      const cacheKey = JSON.stringify(
        value.queries.map((query) => [query.queryKey, query.enabled ?? true]),
      );
      if (cacheKey === instanceQueryCacheKey) {
        return instanceQueryCacheResult;
      }
      instanceQueryCacheKey = cacheKey;
      instanceQueryCacheResult = value.queries.map((query) => {
        const [, resource, provider] = query.queryKey;
        if (resource !== "models") {
          throw new Error(`Unexpected provider instance query: ${String(resource)}`);
        }
        return modelQueries.get(provider as ProviderKind) ?? EMPTY_QUERY;
      });
      return instanceQueryCacheResult;
