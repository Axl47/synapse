import { ThreadId, type ModelSelection, type ProviderKind } from "@synara/contracts";
...
describe("composerDraftStore sticky composer settings", () => {
  ...
  it("preserves Claude Auto support through sticky updates, options, and hydration", () => {
    ...
    expect(
      hydratedClaudeSelection?.instanceId === "claudeAgent"
  ...
  it("preserves current sticky model fields during storage-version migration", () => {
    ...
    const migratedState = persistApi.getOptions().migrate(
      ...
      stickyModelSelectionByProvider: Partial<Record<ModelSelection["instanceId"], ModelSelection>>;
      stickyActiveProvider: ModelSelection["instanceId"] | null;
    };
