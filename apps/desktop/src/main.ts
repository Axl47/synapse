...
async function ensureBrowserHostPipeServer(): Promise<void> {
  ...
  const server = new BrowserHostPipeServer(browserManager, {
    ...
    requestOpenPanel: (threadId) => {
      if (!threadId) return;
      safeSendToWebContents(mainWindow?.webContents, IPC.browser.requestOpenPanel, { threadId });
    },
  });
  await server.start();
...
function emitDesktopWindowState(window: BrowserWindow | null = mainWindow): void {
  ...
  safeSendToWebContents(window.webContents, IPC.windowState, getDesktopWindowState(window));
}

function getDesktopWindowUrl(): string {
  const devServerUrl = process.env.VITE_DEV_SERVER_URL;
  return isDevelopment && devServerUrl ? devServerUrl : desktopIdentity.entryUrl;
}
...
function dispatchMenuAction(action: string): void {
  ...
  const send = () => {
    ...
    safeSendToWebContents(targetWindow.webContents, IPC.menuAction, action);
...
function sendDesktopZoomFactor(webContents: Electron.WebContents): void {
  ...
  safeSendToWebContents(webContents, IPC.zoomFactorChanged, webContents.getZoomFactor());
}
...
function emitUpdateState(): void {
  for (const window of BrowserWindow.getAllWindows()) {
    ...
    safeSendToWebContents(window.webContents, IPC.updateState, updateState);
  }
}
...
function registerIpcHandlers(): void {
  ...
  ipcMain.handle(IPC.windowToggleMaximize, async (event) => {
    ...
    safeSendToWebContents(window.webContents, IPC.windowState, state);
    return state;
  });
