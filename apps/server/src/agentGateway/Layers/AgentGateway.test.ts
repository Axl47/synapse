  ...
  it.effect("returns provider-specific target option keys before the model catalog", () => {
    ...
    return Effect.gen(function* () {
      ...
      const antigravity = targetConstruction.antigravity as {
        ...
        exampleTarget: { options: ReadonlyArray<{ id: string; value: unknown }> };
    ...
    () => {
      const threads = [
        ...
        makeThreadShell("thread-other", {
          ...
          modelSelection: { instanceId: "claudeAgent", model: "opus-4.8" },
  ...
  it.effect("creates a standalone cross-provider thread and dispatches the initial turn", () => {
    ...
    return Effect.gen(function* () {
      ...
      if (create.type === "thread.create") {
        ...
        assert.equal(create.modelSelection.instanceId, "grok");
        ...
        const shell = makeThreadShell(threadId, {
          modelSelection:
            index === 0
              ...
              : { instanceId: "claudeAgent", model: "claude-sonnet-5" },
          latestTurn: {
            turnId: runId,
            state: "completed",
            requestedAt: NOW,
