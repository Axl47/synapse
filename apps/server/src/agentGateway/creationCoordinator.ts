...
function interactionModeForGatewayTarget(target: ModelSelection): ProviderInteractionMode {
  ...
  if (
    ...
    getModelSelectionStringOptionValue(target, "agent") === "plan"
  ) {
    return "plan";
  }
  return "default";
    ...
    return Effect.gen(function* () {
      ...
      const prepared = yield* Effect.forEach(input.threads, (spec, index) =>
        Effect.gen(function* () {
          ...
          const targetProvider = inferLegacyProviderKindFromModelSelection(spec.target);
          const target = yield* resolveAgentGatewayTarget({
            target: spec.target,
            discovery: providerDiscovery,
            ...(providerAvailabilities.get(targetProvider) !== undefined
              ? { availability: providerAvailabilities.get(targetProvider)! }
              : {}),
              ...
              (entry) =>
                Effect.gen(function* () {
                  ...
                  return {
                    ...
                    title: entry.title,
                    target: entry.target,
                    provider: inferLegacyProviderKindFromModelSelection(entry.target),
