...
type ProviderTargetOptionKey<_P extends ProviderKind> = string;
...
function exampleOptionsForRules(
  ...
  return value === null
    ? []
    : [{ id: exampleRule.key, value: typeof value === "number" ? String(value) : value }];
}
...
function failUnavailableOption(
  ...
  const provider = inferLegacyProviderKindFromModelSelection(target);
  throw new AgentGatewayTargetError(
    "model_option_unavailable",
    `Option "${option}" is not available for ${provider}/${target.model}.${
      available && available.length > 0 ? ` Available values: ${available.join(", ")}.` : ""
    }`,
    { provider, model: target.model, option, available: available ?? [] },
  );
}
...
function validateOptionsWithoutCatalog(target: ModelSelection): void {
  const provider = inferLegacyProviderKindFromModelSelection(target);
  for (const { id: optionId, value } of target.options ?? []) {
    if (value === undefined) continue;
    const rule = providerOptionRuleSpec(provider, optionId);
...
export function resolveAgentGatewayTarget(input: {
  ...
  return Effect.gen(function* () {
    const provider = inferLegacyProviderKindFromModelSelection(input.target);
    ...
    if (catalog.models.length > 0 && descriptor === undefined) {
      return yield* Effect.fail(
        new AgentGatewayTargetError(
          "model_unavailable",
          `Model "${input.target.model}" is not available for ${provider}. Use an exact slug from synara_capabilities.`,
          {
            provider,
    ...
    if (catalog.models.length === 0) {
      if (catalog.defaultModel === null) {
        return yield* Effect.fail(
          new AgentGatewayTargetError(
            "provider_unavailable",
            `Provider "${provider}" has no available model catalog or configured default.`,
            { provider, discoveryError: catalog.error },
          ),
        );
      }
      if (input.target.model !== catalog.defaultModel) {
        return yield* Effect.fail(
          new AgentGatewayTargetError(
            "model_unavailable",
            `The ${provider} model catalog is unavailable. Only its configured default "${catalog.defaultModel}" can be used safely; custom model "${input.target.model}" was not verified.`,
            { provider, requestedModel: input.target.model },
          ),
        );
      }
      try {
