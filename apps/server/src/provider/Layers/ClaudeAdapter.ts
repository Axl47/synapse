...
function makeClaudeAdapter(options?: ClaudeAdapterLiveOptions) {
  return Effect.gen(function* () {
    ...
    const startSessionUnlocked: ClaudeAdapterShape["startSession"] = (input) =>
      Effect.gen(function* () {
        ...
        const requestedAutoCompactWindow = trimOrNull(
          getModelSelectionStringOptionValue(modelSelection, "autoCompactWindow") ??
    ...
    const sendTurn: ClaudeAdapterShape["sendTurn"] = (input) =>
      Effect.gen(function* () {
        ...
        if (modelSelection) {
          ...
          const requestedEffortOption = trimOrNull(
    ...
    // Native discovery caches — avoid spawning a process per query and keep
    // account-specific environments isolated from one another.
    ...
    const listCommands: NonNullable<ClaudeAdapterShape["listCommands"]> = (
      ...
      Effect.gen(function* () {
        ...
        const claudeSdkEnv = claudeInstanceEnv(input, input.instanceId);
