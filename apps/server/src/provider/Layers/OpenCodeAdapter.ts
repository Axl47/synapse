    ...
    Effect.gen(function* () {
      ...
      const startSession: OpenCodeAdapterShape["startSession"] = Effect.fn("startSession")(
        function* (input) {
          ...
          if (existing) {
            const wasStopped = yield* Ref.get(existing.stopped);
            yield* stopOpenCodeContext(existing);
            sessions.delete(input.threadId);
            if (!wasStopped) {
              yield* emit(existing, {
                ...buildEventBase({ threadId: input.threadId }),
                providerInstanceId: existing.session.providerInstanceId,
                type: "session.exited",
                payload: {
                  reason: "Session replaced by a new provider instance.",
                  recoverable: false,
                  exitKind: "graceful",
                },
              });
      ...
      const sendTurn: OpenCodeAdapterShape["sendTurn"] = Effect.fn("sendTurn")(function* (input) {
        ...
        const requestedModelSelection =
          inferLegacyProviderKindFromModelSelection(input.modelSelection) === provider
            ? input.modelSelection
            : undefined;
        const modelSelection =
          requestedModelSelection ??
          (context.session.model
            ? { instanceId: provider, model: context.session.model, options: [] }
            : undefined);
        ...
        Effect.gen(function* () {
          ...
          return yield* Effect.scoped(
            Effect.gen(function* () {
              ...
              const explicitServerPassword = input.serverPassword?.trim();
              const serverPassword = explicitServerPassword
