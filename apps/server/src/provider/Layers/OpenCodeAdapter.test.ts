...
function createMockOpenCodeRuntime(options?: {
  ...
  const createClientCalls = sdkClientCalls;
...
describe("OpenCodeAdapter runtime lifecycle", () => {
  ...
  it.each(["opencode", "kilo"] as const)(
    "keeps a reconnecting %s account isolated when the thread switches accounts",
    async (provider) => {
      const failingEvents = createFailingSubscribedEventStream();
      const reconnectedEvents = createSubscribedEventQueue();
      const runtime = createMockOpenCodeRuntime();
      ...
      client.event.subscribe = async () => ({
        stream: subscriptionCount++ === 0 ? failingEvents.stream : reconnectedEvents.stream,
      });
        ...
        Effect.gen(function* () {
          ...
          yield* Effect.gen(function* () {
            for (let attempt = 0; attempt < 100; attempt += 1) {
              if (subscriptionCount >= 2) return;
              yield* Effect.sleep("5 millis");
            }
            throw new Error("Expected the failed account A event stream to reconnect.");
          });
      ...
      const runtimeWarning = events.find((event) => event.type === "runtime.warning");
      const exited = events.find((event) => event.type === "session.exited");
      const rebound = events.filter((event) => event.type === "session.started").at(-1);
      expect(runtimeWarning?.providerInstanceId).toBe(accountA);
      expect(exited?.providerInstanceId).toBe(accountA);
      expect(rebound?.providerInstanceId).toBe(accountB);
    },
  );
    ...
    await Effect.runPromise(
      Effect.gen(function* () {
        ...
        yield* adapter.sendTurn({
          ...
          input: "coordinate first",
          attachments: [],
          modelSelection: { instanceId: "kilo", model: "openai/gpt-5" },
        });
        const secondTurn = yield* adapter
          .sendTurn({
            threadId: secondThread,
    ...
    await Effect.runPromise(
      Effect.gen(function* () {
        ...
        yield* adapter.sendTurn({
          ...
          modelSelection: { instanceId: "opencode", model: "openai/gpt-5.4" },
        });
      }).pipe(
        Effect.provide(
          makeOpenCodeAdapterLive({ runtime: runtime.runtime }).pipe(
