...
export function buildAcpSpawnProcessEnv(
  ...
  const providerEnvironment = spawn.providerEnvironment;
  if (!providerEnvironment) {
    return { ...baseEnv, ...spawn.env };
  }
  return buildProviderProcessEnv({
    driver: providerEnvironment.driver,
    env: baseEnv,
    ...(providerEnvironment.environment !== undefined
      ? { environment: providerEnvironment.environment }
      : {}),
    ...(providerEnvironment.instanceId !== undefined
      ? { instanceId: providerEnvironment.instanceId }
      : {}),
    ...(providerEnvironment.homeDir !== undefined ? { homeDir: providerEnvironment.homeDir } : {}),
    ...(providerEnvironment.isolationRootDir !== undefined
      ? { isolationRootDir: providerEnvironment.isolationRootDir }
      : {}),
    ...(spawn.env !== undefined ? { overlay: spawn.env as Record<string, string> } : {}),
  });
}
...
> =>
  Effect.gen(function* () {
    ...
    const env = buildProviderChildEnvironment({
      provider: "acp",
      baseEnv: {
        ...buildAcpSpawnProcessEnv(options.spawn),
