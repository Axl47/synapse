        ...
        [ORCHESTRATION_WS_METHODS.subscribeThread]: (input, { clientId }) =>
          streamAdmission.guard(
            ...
            makeCursorSafeSnapshotLiveStream({
              ...
              // Adopted Codex threads remain externally writable. Reconcile them immediately
              // before the cursor-safe snapshot so opening or reconnecting a thread observes
              // messages added outside Synara. A provider read failure must not hide the last
              // durable local projection from the user.
              snapshot: syncCodexAdoptedThreadHistory(input.threadId).pipe(
