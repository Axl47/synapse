  ...
  it.effect("keeps a private database that already recorded old migration 54 compatible", () =>
    Effect.gen(function* () {
      ...
      assert.deepStrictEqual(tracker.slice(-43), [
        ...
        { migration_id: 96, name: "ComposerDraftImports" },
      ]);
...
describe("migration lineage aliases", () => {
  it("keeps every declared alias consistent with the current lineage", () => {
    for (const alias of MIGRATION_LINEAGE_ALIASES) {
      ...
      // The historical slot must still be a real divergence and remain occupied.
      // The reviewed alias decides whether that row is renamed or removed.
  ...
  it("recognizes the released fork 70-77 lineage and schedules canonical reruns", () => {
    const recorded = canonicalTrackerThrough(69);
    for (const [id, name] of [
      ...
      recorded.set(id, name);
    }

    assert.deepStrictEqual(
      planMigrationLineageAliasRepairs(recorded),
      Array.from({ length: 8 }, (_, index) => ({
        kind: "remove" as const,
        migrationId: 70 + index,
      })),
    );
  });
