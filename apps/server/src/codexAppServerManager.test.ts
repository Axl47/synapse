    ...
    try {
      ...
      const { env } = (
        manager as unknown as {
          buildSessionLaunchContext: (
            input: { readonly homePath?: string },
            token: string | undefined,
          ) => { readonly env: NodeJS.ProcessEnv };
        }
      ).buildSessionLaunchContext({ homePath }, "token");
...
describe("startSession", () => {
  ...
  it("keeps a stopping session unroutable during adapter-style event inspection", async () => {
