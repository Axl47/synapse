...
If multiple changes were requested and only some were implemented, clearly mark what was completed, what is partial, and what is pending.

Include a Conventional Commit style commit message after each implementation or fix. If it's a large change, follow this format:
...
```mentation): document updater prompt behavior and changelog contract
```
